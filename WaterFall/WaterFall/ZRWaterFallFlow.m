...
@end
@implementation ZRWaterFallFlow
...
- (void)prepareLayout {
    ...
    //_colArr 用来存放相邻的三个高度
    CGFloat top = .0f;
    for (NSUInteger i = 0; i < kColCount; i++) {
...
- (void)layoutItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (NSUInteger i = 0; i < _colArr.count; i ++) {
        CGFloat height = [_colArr[i] floatValue];
...
- (CGSize)collectionViewContentSize {
    ...
    //查找最高的列的高度
    for (NSUInteger i = 0; i < _colArr.count; i++) {
        CGFloat colHeight = [_colArr[i] floatValue];
